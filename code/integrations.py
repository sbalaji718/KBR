...
def long_integration(int_count, minA, maxA, minE, maxE, integration_times, Nparticles, long_int_file= False):
    ...
    sourcePath = '/data/users/arhermos/code/' 
    destinPath = '/data/users/arhermos/Long_Integrations/'
...
def short_integration(int_count, simarchive, sim_length, indexSimulation, filename):
    ...
    sim.automateSimulationArchive("{}/{}".format(subDirTemp,short_filename), IT/Nout, deletefile=True)
...
def check_resonance_make_plots(short_filename):
    ...
    # first for resonant
    random_resonant = np.random.choice(resonant_particles, int(0.05*len(resonant_particles)), replace = False)
    random_nonresonant = np.random.choice(nonresonant_particles, int(0.05*len(nonresonant_particles)), replace = False)
...
def kozai_integration(int_count, simarchive, sim_length, indexSimulation, filename):
    ...
    sim.automateSimulationArchive("{}/{}".format(kozDir,kozai_filename), IT/Nout, deletefile=True)
    sim.integrate(ET, exact_finish_time = 0)
...
def check_kozai_make_plots(short_filename,kozai_filename):
    ...
    df.to_csv("{}/{}".format(subDirTemp,short_filename))
